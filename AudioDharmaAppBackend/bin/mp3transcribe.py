...
#
# mp3transcribe.py
# transcribe mp3s into text files
#
...
# this socket is used as a semaphore, to ensure only one instance of this progrm  runs at a time
ServerSocket = None
HOST = '127.0.0.1'  
PORT = 12345        

MS_IN_MINUTES = 60 * 1000
AUDIO_MAX_WHISPER_DURATION = 60 * MS_IN_MINUTES
AUDIO_CHUNK_SIZE = 30 * MS_IN_MINUTES
#AUDIO_MAX_WHISPER_DURATION = 240 * MS_IN_MINUTES
...
#
# solit the mp3 at path_mp3 into chunks, each of AUDIO_CHUNK_SIZE length
#
def split_mp3(path_mp3, duration):
    ...
    chunk_count = int(duration / AUDIO_CHUNK_SIZE)
    remainder = int(duration % AUDIO_CHUNK_SIZE)
    ...
    for i in range(chunk_count):
        ...
        start_time = i * AUDIO_CHUNK_SIZE
        end_time = start_time + AUDIO_CHUNK_SIZE
    ...
    if remainder > 0:
        ...
        start_time = chunk_count * AUDIO_CHUNK_SIZE
...
# creating new runs.  we do this to balance load across time
#
...
for talk in list_talks:
    ...
    LOG(f"transcribing: {path_mp3} duration: {round(duration / 60000)}")
