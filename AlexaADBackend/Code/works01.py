...
SPEECH_WELCOME = "Welcome to Audio Dharma. Here are the two most recent talks.  <emphasis>These</emphasis> are followed by four suggested talks<break time=\"1s\"/>"
...
#
def jsonAnnouncement(talkTitle, speech, repromptSpeech, shouldEndSession):
    return {
        'outputSpeech': {
            'type': 'SSML',
            'ssml': speech
        ...
        'card': {
            'type': 'Simple',
            'title': 'SessionSpeechlet - ' + talkTitle,
...
def jsonPlayTalk(talkTitle, output, talkURL):
    return {
        'outputSpeech': {
            'type': 'SSML',
            'ssml': output
...
def jsonCancelSession(talkTitle, output, talkURL):
    return {
        'outputSpeech': {
            'type': 'SSML',
            'ssml': output
...
def welcomeResponse():
    ...
    for talk in recentTalksList:
        number = "<emphasis>" + talk['number'] + "</emphasis>"
        title = talk['title']
...
def unsupportedLooping():
    ...
    response = buildResponse(jsonAnnouncement(cardTitle, speechOutput, reprompt_text, shouldEndSession))
    print(response)
    return response
...
def cancelSession():
    ...
    response = buildResponse(jsonCancelSession(cardTitle, speechOutput, None))
    print(response)
    return response
...
def playTalk(message, talkIndex, recentTalksList):
    ...
    cardTitle = message + ": "  + talkTitle
    speechOutput = speakString(cardTitle)
    response =  buildResponse(jsonPlayTalk(cardTitle, speechOutput, talkURL))
    print(response)
    return response
...
def loadMostRecentTalks():
    ...
    for talk in talks:
        ...
        talkAttributes = {'number': number, 'title': title, 'speaker': speaker, 'url': url, 'minutes': minutes}
